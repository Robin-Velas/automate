...
namespace fa {
  ...
  public:
    ...
    struct aut_state {
        ...
        bool isFinal;
        /*
         * Create a map representing the transitions :
         * key (char) is used for a letter of the alphabet
         * set<int> will represent the possible next states 
         * for exemple, "abb" will create <'a', {1, 2}> if there are 2 possibles states for 'a'.
         */
        std::map<char, std::set<int>> transitions;
    };
    
    std::vector<char> alphabet;
    std::map<int, struct aut_state> allStates;
  };
